...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const all = res.data || [];
        setMeasures(all.filter(m => isSchipperMaatregel(m.id)));
      } catch (e) {
  ...
  const toggle30 = (code) => {
    setSelected30(prev => { const next = { ...prev }; if (next[code] !== undefined) delete next[code]; else next[code] = 1; return next; });
    setCalcResult30(null);
  };
  const toggle50 = (code) => {
    setSelected50(prev => { const next = { ...prev }; if (next[code] !== undefined) delete next[code]; else next[code] = 1; return next; });
    setCalcResult50(null);
  };
  ...
  const calculate = async () => {
    ...
    try {
      const promises = [];
      if (selected30Codes.length > 0) promises.push(apiPost("/measures/subsidies", { type, measures: selected30Codes.map(code => ({ code, quantity: selected30[code] || 0 })) }));
      else promises.push(Promise.resolve(null));
      if (selected50Codes.length > 0) promises.push(apiPost("/measures/subsidies", { type, measures: selected50Codes.map(code => ({ code, quantity: selected50[code] || 0 })) }));
      else promises.push(Promise.resolve(null));
      const [res30, res50] = await Promise.all(promises);
      setCalcResult30(res30); setCalcResult50(res50); setActiveTab("resultaat");
    } catch (e) { setCalcError(e.message); } finally { setCalcLoading(false); }
  };

  const downloadPDF = async (regelingType) => {
    const codes = regelingType === "30" ? selected30Codes : selected50Codes;
    const sel = regelingType === "30" ? selected30 : selected50;
    if (!codes.length) return;
    setPdfLoading(true);
    try {
      const r = await fetch(`${API}/measures/generate_pdf`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type, measures: codes.map(code => ({ code, quantity: sel[code] || 0 })) }) });
      if (!r.ok) throw new Error("PDF genereren mislukt");
      const blob = await r.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url; a.download = `subsidieadvies-${regelingType}pct-${klant.replace(/\s+/g, "-") || "klant"}.pdf`; a.click();
  ...
  const totaal50Subsidie = totaal50Catalogus != null ? totaal50Catalogus * (regeling === "100" ? 1.0 : 0.5) : null;
  const totaalSubsidie = (totaal30Subsidie ?? 0) + (totaal50Subsidie ?? 0);
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <div style={{ display: "flex", background: "rgba(255,255,255,0.12)", borderRadius: 8, overflow: "hidden", border: "1px solid rgba(255,255,255,0.2)" }}>
            {[["diy", "Doe-het-zelf"], ["contractor", "Aannemer"]].map(([v, label]) => (
              <button key={v} onClick={() => setType(v)} style={{ padding: "7px 14px", border: "none", background: type === v ? "rgba(255,255,255,0.25)" : "transparent", color: "white", fontSize: 12, fontWeight: type === v ? 700 : 400, cursor: "pointer" }}>
                {label}
              </button>
            ))}
          </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", minHeight: "calc(100vh - 56px)" }}>
        <div style={{ padding: "20px 24px", overflow: "auto" }}>
          ...
          <div style={{ display: "flex", gap: 4, marginBottom: 16 }}>
            {[["invoer", "📋 Maatregelen invoeren"], ["resultaat", `📊 Resultaat${heeftResultaat && totaalSubsidie > 0 ? ` · € ${totaalSubsidie.toLocaleString("nl-NL", { minimumFractionDigits: 2 })}` : ""}`]].map(([t, label]) => (
              ...
              <div style={{ background: "white", borderRadius: 14, overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
                <div style={{ background: "linear-gradient(135deg,#7d5a00,#c49a00)", color: "white", padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div>
                    ...
                    <div style={{ fontSize: 11, opacity: 0.8, marginTop: 2 }}>Vervangen kozijn met triple glas · dakramen</div>
                  </div>
                  <div style={{ background: "rgba(255,255,255,0.2)", borderRadius: 8, padding: "4px 12px", fontSize: 12, fontWeight: 700 }}>
                    {selected30Codes.length} geselecteerd
                  </div>
                </div>
                  ...
                  {Object.entries(grouped30).map(([sub, items]) => (
                    <div key={sub}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                        {items.map(m => (
                          <MaatrgelKaart key={m.id} m={m} isSelected={selected30[m.id] !== undefined} isDisabled={false} onToggle={toggle30} onQtyChange={setQty30} qty={selected30[m.id]} type={type} color="#c49a00" />
                        ))}
                      </div>
                    </div>
                  ))}
              ...
              <div style={{ background: "white", borderRadius: 14, overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
                <div style={{ background: "linear-gradient(135deg,#1B4D3E,#2D6A4F)", color: "white", padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div>
                    ...
                    <div style={{ fontSize: 11, opacity: 0.8, marginTop: 2 }}>HR++ glas · kierdichting · bijkomende kosten</div>
                  </div>
                  <div style={{ background: "rgba(255,255,255,0.2)", borderRadius: 8, padding: "4px 12px", fontSize: 12, fontWeight: 700 }}>
                    {selected50Codes.length} geselecteerd
                  </div>
                </div>
                <div style={{ padding: "14px 20px 0", display: "flex", gap: 8 }}>
                  {[["50", "50% regeling"], ["100", "100% regeling"]].map(([v, label]) => (
                    <button key={v} onClick={() => setRegeling(v)} style={{ flex: 1, padding: "9px", borderRadius: 9, border: regeling === v ? "2px solid #1B4D3E" : "1.5px solid #dde", background: regeling === v ? "#1B4D3E" : "white", color: regeling === v ? "white" : "#555", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
                      {label}
                    </button>
                  ))}
                </div>
                <div style={{ padding: "14px 20px", display: "flex", flexDirection: "column", gap: 14 }}>
                  {Object.entries(grouped50).map(([sub, items]) => (
                    <div key={sub}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                        {items.map(m => (
                          <MaatrgelKaart key={m.id} m={m} isSelected={selected50[m.id] !== undefined} isDisabled={false} onToggle={toggle50} onQtyChange={setQty50} qty={selected50[m.id]} type={type} color="#2D6A4F" />
                        ))}
                      </div>
                    </div>
                  ))}
          ...
          {activeTab === "resultaat" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              {calcError && (
                <div style={{ background: "#fff0f0", border: "1px solid #f5c6c6", borderRadius: 10, padding: "14px 18px", color: "#c0392b", fontSize: 13 }}>⚠️ {calcError}</div>
              )}
              {calcResult30 && (
                <div style={{ background: "white", borderRadius: 14, overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
                  <div style={{ background: "linear-gradient(135deg,#7d5a00,#c49a00)", color: "white", padding: "14px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 22, fontWeight: 800 }}>€ {totaal30Subsidie?.toLocaleString("nl-NL", { minimumFractionDigits: 2 })}</div>
                    </div>
                  </div>
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                      <tr style={{ background: "#fdf8e8" }}>
                        {["Maatregel", "Hoev.", "Catalogus", "Subsidie 30%"].map(h => (
                          <th key={h} style={{ padding: "8px 14px", textAlign: h === "Maatregel" ? "left" : "right", fontSize: 10, fontWeight: 700, color: "#7d5a00", letterSpacing: "0.06em", textTransform: "uppercase" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {calcResult30.data.map((row, i) => (
                        <tr key={row.id} style={{ borderBottom: "1px solid #f5f0e0", background: i % 2 === 0 ? "#fffef8" : "white" }}>
                          <td style={{ padding: "9px 14px" }}>
                            <div style={{ fontWeight: 600, fontSize: 12 }}>{row.attributes.name}</div>
                            <div style={{ fontSize: 10, color: "#bbb" }}>{row.id}</div>
                          </td>
                  ...
                  <div style={{ padding: "12px 20px" }}>
                    <button onClick={() => downloadPDF("30")} disabled={pdfLoading} style={{ background: "#c49a00", color: "white", border: "none", borderRadius: 8, padding: "10px 20px", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
                      📄 PDF 30% regeling
                    </button>
                  </div>
                </div>
              ...
              {calcResult50 && (
                <div style={{ background: "white", borderRadius: 14, overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
                  <div style={{ background: "linear-gradient(135deg,#1B4D3E,#2D6A4F)", color: "white", padding: "14px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <div style={{ fontWeight: 800, fontSize: 14 }}>{regeling === "100" ? "100%" : "50%"} Regeling — HR++ glas</div>
                      <div style={{ fontSize: 11, opacity: 0.8 }}>Cataloguswaarde: € {totaal50Catalogus?.toLocaleString("nl-NL", { minimumFractionDigits: 2 })}</div>
                    </div>
                    <div>
                      <div style={{ fontSize: 10, opacity: 0.8 }}>Subsidie ({regeling === "100" ? "100" : "50"}%)</div>
                      <div style={{ fontSize: 22, fontWeight: 800 }}>€ {totaal50Subsidie?.toLocaleString("nl-NL", { minimumFractionDigits: 2 })}</div>
                    </div>
                  </div>
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                      <tr style={{ background: "#f0f8f4" }}>
                        {["Maatregel", "Hoev.", "Catalogus", `Subsidie ${regeling === "100" ? "100" : "50"}%`].map(h => (
                          <th key={h} style={{ padding: "8px 14px", textAlign: h === "Maatregel" ? "left" : "right", fontSize: 10, fontWeight: 700, color: "#1B4D3E", letterSpacing: "0.06em", textTransform: "uppercase" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {calcResult50.data.map((row, i) => {
                        const pct = regeling === "100" ? 1.0 : 0.5;
                        return (
                          <tr key={row.id} style={{ borderBottom: "1px solid #eef2ee", background: i % 2 === 0 ? "#fafcfa" : "white" }}>
                            ...
                            <td style={{ padding: "9px 14px", textAlign: "right", fontSize: 12 }}>{row.attributes.quantity} {row.attributes.unit}</td>
                            <td style={{ padding: "9px 14px", textAlign: "right", fontSize: 12 }}>€ {row.attributes.totalForQuantity?.toFixed(2)}</td>
                            <td style={{ padding: "9px 14px", textAlign: "right", fontWeight: 700, fontSize: 13, color: "#1B4D3E" }}>€ {(row.attributes.totalForQuantity * pct).toFixed(2)}</td>
                          </tr>
                        );
                      })}
                    </tbody>
                  ...
                  <div style={{ padding: "12px 20px" }}>
                    <button onClick={() => downloadPDF("50")} disabled={pdfLoading} style={{ background: "#1B4D3E", color: "white", border: "none", borderRadius: 8, padding: "10px 20px", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
                      📄 PDF {regeling === "100" ? "100%" : "50%"} regeling
                    </button>
                  </div>
                </div>
              )}
        ...
        <div style={{ background: "white", borderLeft: "1px solid #e0e8e0", padding: 20, display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div style={{ borderRadius: 10, padding: "12px 14px", background: "#fffbf0", border: "1.5px solid #f0c040" }}>
            ...
            <div style={{ fontSize: 22, fontWeight: 800, color: "#7d5a00" }}>
              {totaal30Subsidie != null ? `€ ${totaal30Subsidie.toLocaleString("nl-NL", { minimumFractionDigits: 2 })}` : `${selected30Codes.length} maatregel${selected30Codes.length !== 1 ? "en" : ""}`}
            </div>
          ...
          <div style={{ borderRadius: 10, padding: "12px 14px", background: "#eef8f2", border: "1.5px solid #2D6A4F" }}>
            ...
            <div style={{ fontSize: 22, fontWeight: 800, color: "#1B4D3E" }}>
              {totaal50Subsidie != null ? `€ ${totaal50Subsidie.toLocaleString("nl-NL", { minimumFractionDigits: 2 })}` : `${selected50Codes.length} maatregel${selected50Codes.length !== 1 ? "en" : ""}`}
            </div>
          ...
          {heeftResultaat && totaalSubsidie > 0 && (
            <div style={{ background: "linear-gradient(135deg,#0d2e1f,#1B4D3E)", color: "white", borderRadius: 12, padding: "16px 18px" }}>
              <div style={{ fontSize: 10, opacity: 0.7, marginBottom: 4 }}>Totale subsidie klant</div>
              <div style={{ fontSize: 28, fontWeight: 800 }}>€ {totaalSubsidie.toLocaleString("nl-NL", { minimumFractionDigits: 2 })}</div>
            </div>
          )}
