...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const mRes = await apiFetch("/measures");
        const allMeasures = mRes.data || [];
        setMeasures(allMeasures.filter(m => isSchipper(m.id)));

        // Bijkomende kosten halen we rechtstreeks uit de al-opgehaalde /measures lijst —
        // elke measure heeft zijn eigen additionalCosts array nested in attributes,
        // dus een aparte /measures/{id} fetch per familie is niet nodig (en V1-2-A1 bestaat
        // sowieso niet als los endpoint, dat gaf een 404).
        const findMeasure = (id) => allMeasures.find(m => m.id === id);
